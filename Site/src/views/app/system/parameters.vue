<template>
  ...
  <div class="main-content">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page"><strong>{{ $t('parameters') }}</strong></li>
    </ol>
      ...
      <b-container fluid>
        <b-form @submit="saveRecord" @reset="onReset" id="formAddRecord">
          <b-form-group class="col-md-12 mb-3" id="input-group-1" label="Parameter" label-for="input-1">
            <b-form-input id="input-1" v-model="form.syp_name" required placeholder="Enter parameter name"/>
          </b-form-group>

          <b-form-group class="col-md-12 mb-3" id="input-group-2" label="Description" label-for="input-2">
            <b-form-input id="input-2" v-model="form.syp_description" required placeholder="Enter parameter description"/>
          </b-form-group>

          <b-form-group class="col-md-12 mb-3" id="input-group-3" label="Value" label-for="input-3">
            <b-form-input id="input-3" v-model="form.syp_value" required placeholder="Enter parameter value"/>
          </b-form-group>
    ...
    <b-modal  v-model="showUpdateModal"  title="Editing record"  >
      <b-container fluid>              
        <b-form @submit="updateRecord" @reset="onReset" id="formUpdateRecord">
          <b-row>
            <b-form-group class="col-md-12 mb-3" id="input-group-1" label="Parameter" label-for="input-1">
              <b-form-input id="input-1" v-model="form.syp_name" required readonly placeholder="Enter parameter name"/>
            </b-form-group>
            ...
            <b-form-group class="col-md-12 mb-3" id="input-group-3" >
              <b-form-checkbox id="checkbox-1" v-model="form.syp_status" name="checkbox-1"  value="On" unchecked-value="Off">
                Status: <strong>{{ form.syp_status }}</strong>
              </b-form-checkbox>               
            </b-form-group>
          ...
          >   
            ...
            <template slot="table-row" slot-scope="props">
              <span v-if="props.column.field == 'actions'">
                  ...
                  <i class="i-Close-Window text-25 text-danger" @click="showDeleteRecordModal(props.row.syp_id)" style="cursor: pointer;"></i>
              </span>
            </template>
          </vue-good-table>
        </b-card>
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {
            label: 'Create date',
            field: 'syp_create_date',
          },
          {
            label: 'Update date',
            field: 'syp_update_date',
          },
          {
            label: 'Actions',
            field: 'actions',
    ...
    methods: {
      ...
      saveRecord(evt) {
...

       {
          ...
          {
            "syp_name": this.form.syp_name,
            "syp_description": this.form.syp_description,
            "syp_value": this.form.syp_value,
            "user": username
          },
          { 
            headers: { 
      ...
      updateRecord(evt){
        ...
        axios.put(apiServer+'/api/'+this.table+'/'+this.form.syp_id,   
          {
            "syp_description": this.form.syp_description,
            "syp_value": this.form.syp_value,
            "syp_status": this.form.syp_status,
            "user": username
          },
          { 
            headers: { 
      ...
      showUpdateRecordModal(userInfo){
        this.form.syp_id = userInfo.syp_id
        this.form.syp_name = userInfo.syp_name
        this.form.syp_description = userInfo.syp_description
        this.form.syp_value = userInfo.syp_value
        this.form.syp_status = userInfo.syp_status
