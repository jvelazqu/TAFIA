<template>
  ...
  <div class="main-content">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page"><strong>{{ $t('groups') }}</strong></li>
    </ol>
    ...
    <b-modal  v-model="showAddModal" title="Adding record" hide-footer>
      <b-container fluid>
        <b-form @submit="saveRecord" @reset="onReset" id="formAddRecord">
          <b-form-group class="col-md-12 mb-3" id="input-group-1" label="Group" label-for="input-1">
            <b-form-input id="input-1" v-model="form.grp_name" required placeholder="Enter group name"/>
          </b-form-group>

          <b-form-group class="col-md-12 mb-3" id="input-group-2" label="Description" label-for="input-2">
            <b-form-input id="input-2" v-model="form.grp_description" required placeholder="Enter group description"/>
          </b-form-group>
    ...
    <b-modal  v-model="showUpdateModal"  title="Editing record"  >
      <b-container fluid>              
        <b-form @submit="updateRecord" @reset="onReset" id="formUpdateRecord">
          <b-row>
            <b-form-group class="col-md-12 mb-3" id="input-group-1" label="Group" label-for="input-1">
              <b-form-input id="input-1" v-model="form.grp_name" required readonly placeholder="Enter group name"/>
            </b-form-group>
            ...
            <b-form-group class="col-md-12 mb-3" id="input-group-3" >
              <b-form-checkbox id="checkbox-1" v-model="form.grp_status" name="checkbox-1"  value="On" unchecked-value="Off">
                Status: <strong>{{ form.grp_status }}</strong>
              </b-form-checkbox>               
            </b-form-group>
          ...
          >   
            ...
            <template slot="table-row" slot-scope="props">
              <span v-if="props.column.field == 'actions'">
                  ...
                  <i class="i-Close-Window text-25 text-danger" @click="showDeleteRecordModal(props.row.grp_id)" style="cursor: pointer;"></i>
              </span>
            </template>
          </vue-good-table>
        </b-card>
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {
            label: 'Create date',
            field: 'grp_create_date',
          },
          {
            label: 'Update date',
            field: 'grp_update_date',
          },
          {
            label: 'Actions',
            field: 'actions',
    ...
    methods: {
      ...
      saveRecord(evt) {
...

       {
          ...
          {
            "grp_name": this.form.grp_name,
            "grp_description": this.form.grp_description,
            "user": username
          },
          { 
            headers: { 
      ...
      updateRecord(evt){
        ...
        axios.put(apiServer+'/api/'+this.table+'/'+this.form.grp_id,   
          {
            "grp_description": this.form.grp_description,
            "grp_status": this.form.grp_status,
            "user": username
          },
          { 
            headers: { 
      ...
      showAddRecordModal(){
        ...
        this.form.grp_name = ""
        this.form.grp_description = ""
        this.showAddModal = true
      },

      // Show a modal with information to edit and update it
      showUpdateRecordModal(userInfo){
        this.form.grp_id = userInfo.grp_id
        this.form.grp_name = userInfo.grp_name
        this.form.grp_description = userInfo.grp_description
        this.form.grp_status = userInfo.grp_status
